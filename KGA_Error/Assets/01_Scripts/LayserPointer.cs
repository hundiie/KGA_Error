    ...
    void Update()
    {
        ...
        if (Physics.Raycast(transform.position, transform.forward, out hit, raycastDistance))
        {
            ...
            if (hit.collider.gameObject.CompareTag("Button"))
            {
                ...
                if (OVRInput.GetDown(OVRInput.Button.One) && !hit.transform.GetComponentInParent<ButtonController>().doNotPush) // 클릭하면
                {
                ...
                //else
                //{
                //    // Collided_object.collider.gameObject.GetComponent<Buttonss>().OnPointerEnter(null);
